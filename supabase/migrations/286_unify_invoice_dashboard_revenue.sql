-- Migration 286: Unify invoice dashboard revenue definition
--
-- PROBLEM: get_invoice_dashboard() (migration 137) computed revenue as
--   SUM(total_earned) only, ignoring price_after_discount. Meanwhile the
--   JS helper lineRevenue() and get_performance_invoice_summary() use
--   COALESCE(total_earned, price_after_discount, 0). This made the Practice
--   Analytics "Overview" tab disagree with the CRM and Performance tabs for
--   any line where total_earned is NULL but price_after_discount is set.
--
-- FIX: Recreate get_invoice_dashboard() using the same
--   COALESCE(total_earned, price_after_discount, 0) revenue expression
--   everywhere (stats, monthly, departments, product groups, staff, case
--   owners) so every report agrees on a single revenue definition.
--
-- Columns are NUMERIC(12,2); no casts required. Signature is unchanged.
...
DECLARE
  ...
  v_case_owners jsonb;
BEGIN
  -- Stats summary (exclude Header rows). Revenue uses the unified
  -- COALESCE(total_earned, price_after_discount, 0) definition.
  SELECT jsonb_build_object(
    'totalLines', COUNT(*),
    'totalRevenue', COALESCE(SUM(COALESCE(total_earned, price_after_discount, 0)), 0),
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'revenue', COALESCE(SUM(COALESCE(total_earned, price_after_discount, 0)), 0),
      'lineCount', COUNT(*),
  ...
  FROM (
    ...
    COALESCE(SUM(COALESCE(total_earned, price_after_discount, 0)), 0) AS rev
    FROM invoice_lines
    WHERE (invoice_type IS DISTINCT FROM 'Header')
      AND (p_start_date IS NULL OR invoice_date >= p_start_date)
      AND (p_end_date IS NULL OR invoice_date <= p_end_date)
