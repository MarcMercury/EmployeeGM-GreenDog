 ...
 * This script adds the level_descriptions column and imports skills
 */
...
// Skills data with level descriptions
const skillsData = [
  // Clinical Skills
...
async function applyMigrations() {
  console.log('=== Applying Skills Taxonomy Migrations ===\n')
  
  // Step 1: Check current schema
  console.log('Step 1: Checking current skill_library schema...')
  ...
  if (sampleError) {
    console.error('Error accessing skill_library:', sampleError.message)
    return
  }
  
  const hasLevelDescriptions = sample && sample.length > 0 && 'level_descriptions' in sample[0]
  console.log(`  level_descriptions column exists: ${hasLevelDescriptions}`)
  
  if (!hasLevelDescriptions) {
    console.log('\n⚠️  The level_descriptions column needs to be added.')
...
`)
    console.log('\nOnce the column is added, re-run this script.')
    return
  }
  
  // Step 2: Upsert skills with level descriptions
  console.log('\nStep 2: Importing skills with level descriptions...')
  ...
  // Step 3: Add Skills Library page access
  console.log('\nStep 3: Adding Skills Library page access...')
  
  // Check if page_definitions table exists and add entry
  ...
  if (pageError) {
    console.log(`  Page definition: ${pageError.message}`)
  } else {
    console.log('  Page definition added/updated')
  }
