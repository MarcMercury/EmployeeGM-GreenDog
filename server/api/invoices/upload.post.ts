...
function mapRow(row: Record<string, any>): Record<string, any> {
  return {
    invoice_number: row['Invoice'] || row['invoice'] || row['Invoice Number'] || row['invoice_number'] || null,
    invoice_line_reference: row['Invoice Line Reference'] || row['invoice_line_reference'] || row['Line Reference'] || row['line_reference'] || null,
    invoice_date: parseDate(row['Invoice Line Date Created'] || row['Invoice Line Date'] || row['invoice_date'] || row['Date'] || row['date']),
    invoice_date_modified: parseDate(row['Invoice Line Date Last Modified'] || row['invoice_date_modified'] || row['Date Modified']),
    invoice_time: parseTime(row['Invoice Line Time'] || row['invoice_time'] || row['Time']),
    ...
    standard_price: parseNumber(row['Standard Price'] || row['standard_price'] || row['Unit Price']),
    ...
    price_after_discount: parseNumber(row['Price After Discount'] || row['price_after_discount'] || row['Net Price']),
    total_tax_amount: parseNumber(row['Total Tax Amount'] || row['total_tax_amount'] || row['Tax']),
    total_earned: parseNumber(row['Total Earned'] || row['total_earned'] || row['Total'] || row['Amount']),
