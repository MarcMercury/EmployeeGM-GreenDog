...
const fetchInventoryItems = async () => {
  const { data, error } = await supabase
    ...
    .select('id, item_name, category, total_quantity')
    .order('item_name')
...
// Create linked marketing event for CE events
// This ensures CE events appear on the Marketing Calendar and Events List
// Returns the marketing event ID if created successfully
async function createMarketingEvent(ceEventId: string): Promise<string | null> {
  try {
    // Create linked marketing event - use 'name' field (not 'title') per schema
    const { data, error } = await supabase
      .from('marketing_events')
      .insert({
        name: `[CE] ${formData.value.title}`,
        description: formData.value.description || null,
        event_date: formData.value.event_date_start,
        start_time: null,
        end_time: null,
        event_type: 'ce_event',
        location: formData.value.location_name || null,
        status: 'confirmed',
        staffing_status: 'confirmed',
        expected_attendance: formData.value.max_attendees || null,
        notes: `CE Event - ${formData.value.ce_hours_offered} CE Hours. Speaker: ${formData.value.speaker_name || 'TBD'}. Linked to CE Event ID: ${ceEventId}`
      })
      .select('id')
      .single()
    ...
    console.error('Marketing events dual-write error:', e)
    return null
  }
}
        ...
        if (item.inventory_item_id && item.quantity_used > 0) {
          try {
            await supabase.rpc('deduct_inventory_for_event', {
              ...
              p_notes: `CE Event: ${formData.value.title}`
            })
          } catch (invError) {
          ...
          <v-row>
            ...
            <v-col cols="12">
              ...
              <div class="d-flex align-center mb-3">
                ...
                <v-spacer />
                <v-btn size="small" variant="text" color="warning" @click="addInventoryItemLine">
                  <v-icon start>mdi-plus</v-icon> Add Item
                </v-btn>
              </div>
              
              <v-alert v-if="selectedInventoryItems.length === 0" type="info" variant="tonal" density="compact" class="mb-3">
                Select inventory items (brochures, swag, supplies) to bring to this CE event. Quantities will be deducted from stock when saved.
              </v-alert>
                ...
                <v-row dense align="center">
                  <v-col cols="12" md="5">
                    <v-autocomplete
                      ...
                      density="compact"
                      hide-details
                      @update:model-value="(val) => updateSelectedItemName(index, val)"
                    >
                  ...
                  <v-col cols="3" md="2" style="min-width: 90px;">
                    <v-text-field
