  ...
  <div>
    ...
    <div class="d-flex align-center justify-space-between mb-4">
      <div class="d-flex align-center gap-2">
        ...
        <v-chip v-if="atRiskGoalsCount > 0" color="error" variant="tonal" size="small">
          {{ atRiskGoalsCount }} At Risk
        </v-chip>
      </div>
      <v-btn v-if="isOwnProfile" color="primary" prepend-icon="mdi-plus" size="small" @click="showCreateGoalDialog = true">
        New Goal
      </v-btn>
    </div>
    ...
    <v-row class="mb-4">
      <v-col cols="6" sm="3">
        <v-card variant="tonal" color="primary" class="text-center pa-3" rounded="lg" elevation="0">
          <div class="text-h5 font-weight-bold">{{ activeGoals.length }}</div>
          <div class="text-caption">Active Goals</div>
        </v-card>
      </v-col>
      <v-col cols="6" sm="3">
        <v-card variant="tonal" color="success" class="text-center pa-3" rounded="lg" elevation="0">
          <div class="text-h5 font-weight-bold">{{ onTrackCount }}</div>
          <div class="text-caption">On Track</div>
        </v-card>
      </v-col>
      <v-col cols="6" sm="3">
        <v-card variant="tonal" color="error" class="text-center pa-3" rounded="lg" elevation="0">
          <div class="text-h5 font-weight-bold">{{ atRiskGoalsCount }}</div>
          <div class="text-caption">At Risk</div>
        </v-card>
      </v-col>
      <v-col cols="6" sm="3">
        <v-card variant="tonal" color="grey" class="text-center pa-3" rounded="lg" elevation="0">
          <div class="text-h5 font-weight-bold">{{ completedGoals.length }}</div>
    ...
    <!-- Empty State -->
    <v-card v-if="filteredGoals.length === 0" class="text-center pa-8 bg-white shadow-sm rounded-xl" elevation="0">
      <v-icon size="64" color="grey-lighten-1">mdi-flag-outline</v-icon>
      <h3 class="text-h6 mt-4">No goals yet</h3>
      <p class="text-body-2 text-grey">{{ isOwnProfile ? 'Create your first goal to start tracking progress' : 'No goals have been set' }}</p>
      <v-btn v-if="isOwnProfile" color="primary" class="mt-4" @click="showCreateGoalDialog = true">
    ...
    <!-- Goals List -->
    <v-card v-else class="bg-white shadow-sm rounded-xl" elevation="0">
      <v-list lines="three">
          ...
          <v-list-item @click="openGoal(goal)" class="goal-item py-4">
            <template #prepend>
              <v-avatar :color="getGoalStatusColor(goal)" size="40">
                <v-icon color="white" size="20">{{ getGoalStatusIcon(goal) }}</v-icon>
              </v-avatar>
            </template>
            ...
            <v-list-item-subtitle>
              <div class="d-flex align-center gap-2 mb-2">
                <v-chip :color="getGoalRiskColor(goal)" size="x-small" variant="tonal">
                  {{ getGoalRiskLabel(goal) }}
                </v-chip>
                <v-chip v-if="goal.target_date" size="x-small" variant="outlined">
                  <v-icon start size="12">mdi-calendar</v-icon>
                  {{ formatDate(goal.target_date) }}
                </v-chip>
                <v-chip :color="getVisibilityColor(goal.visibility)" size="x-small" variant="tonal">
                  {{ goal.visibility }}
                </v-chip>
              </div>
              <p class="text-truncate" style="max-width: 500px;">{{ goal.description }}</p>
            </v-list-item-subtitle>
              ...
              <div class="d-flex align-center gap-4">
                <div class="text-center">
                  <v-progress-circular
                    ...
                    :width="6"
                  >
                    <span class="text-caption font-weight-bold">{{ Math.round(goal.progress_percent) }}%</span>
                  </v-progress-circular>
                </div>
    ...
    <v-dialog v-model="showCreateGoalDialog" max-width="600" persistent>
      <v-card>
        ...
        <v-card-text>
          ...
          <v-row>
            <v-col cols="6">
              <v-text-field v-model="newGoal.start_date" label="Start Date" type="date" variant="outlined" />
            </v-col>
            <v-col cols="6">
              <v-text-field v-model="newGoal.target_date" label="Target Date" type="date" variant="outlined" />
            </v-col>
          </v-row>
          <v-select
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="showCreateGoalDialog = false">Cancel</v-btn>
          <v-btn color="primary" :loading="creatingGoal" :disabled="!newGoal.title" @click="createGoal">
            Create Goal
          </v-btn>
        </v-card-actions>
      </v-card>
...
const filteredGoals = computed(() => {
  switch (goalTab.value) {
    ...
    default: return performanceStore.goals
  }
})

const alignmentOptions = computed(() => {
  return performanceStore.goals.filter(g => g.visibility !== 'private' && g.status === 'active')
})
...
async function createGoal() {
  ...
  try {
    ...
    newGoal.value = { title: '', description: '', start_date: '', target_date: '', visibility: 'private', aligns_to_goal_id: null }
  } catch (err: any) {
