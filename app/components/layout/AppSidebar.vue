<template>
  ...
  >
    ...
    >
      ...
      <!-- office_admin and user only see Calendar and Resources (view-only) -->
      <v-list-group v-if="hasMarketingAccess && !rail" value="marketing">
        ...
        <!-- Command Center - Marketing Hub (full access only) -->
        <v-list-item v-if="hasMarketingFullAccess" to="/marketing/command-center" title="Command Center" prepend-icon="mdi-view-dashboard" density="compact" rounded="lg" class="nav-item ml-4" />
        <!-- Calendar - visible to all marketing roles including view-only -->
        ...
        <!-- Events (full access only) -->
        <v-list-item v-if="hasMarketingFullAccess" to="/growth/events" title="Events" prepend-icon="mdi-calendar-star" density="compact" rounded="lg" class="nav-item ml-4" />
        <!-- Marketing Hubs (full access only) -->
        <v-list-item v-if="hasMarketingFullAccess" to="/marketing/partners" title="Partners" prepend-icon="mdi-handshake" density="compact" rounded="lg" class="nav-item ml-4" />
        <v-list-item v-if="hasMarketingFullAccess" to="/marketing/influencers" title="Influencers" prepend-icon="mdi-account-star-outline" density="compact" rounded="lg" class="nav-item ml-4" />
        <v-list-item v-if="hasMarketingFullAccess" to="/marketing/inventory" title="Inventory" prepend-icon="mdi-package-variant" density="compact" rounded="lg" class="nav-item ml-4" />
        <!-- Resources - visible to all marketing roles including view-only -->
      ...
      <!-- CRM & Analytics Group - CRM Access Only (NOT office_admin or user) -->
      ...
      <!-- GDU (Education) Group - Education Access (NOT office_admin or user) -->
...
// Marketing has tiered access: full (edit) vs view (calendar/resources only)
const hasMarketingAccess = computed(() => SECTION_ACCESS.marketing.includes(userRole.value))
const hasMarketingFullAccess = computed(() => SECTION_ACCESS.marketing_full?.includes(userRole.value) ?? false)
const hasCrmAccess = computed(() => SECTION_ACCESS.crm?.includes(userRole.value) ?? false)
